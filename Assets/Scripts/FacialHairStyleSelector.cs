...
public class FacialHairStyleSelector : MonoBehaviour
{
    [System.Serializable]
    public class HairStyle {
        public Image facialHairStyleImage;
        public Sprite facialHairBlankSprite;
    }
    ...
    void OnEnable()
    {
        foreach (HairStyle hairStyle in hairStyles) {
            UpdateCharacterTexture(hairStyle.facialHairBlankSprite, hairStyle.facialHairStyleImage);
        }
    }
    ...
    public Texture2D CopyTexture2D(Texture2D copiedTexture)
    {
        ...
        while (y < texture.height)
        {
            ...
            {
                if(copiedTexture.GetPixel(x,y) == Color.red) texture.SetPixel(x, y, characterCreationData.facialHairColor);
                else if (copiedTexture.GetPixel(x,y) == Color.green) texture.SetPixel(x, y, new Color32(GetValidByteColor(characterCreationData.facialHairColor.r, -15), GetValidByteColor(characterCreationData.facialHairColor.g, -15), GetValidByteColor(characterCreationData.facialHairColor.b, -15), 255));
                else if (copiedTexture.GetPixel(x,y) == Color.blue) texture.SetPixel(x, y, new Color32(GetValidByteColor(characterCreationData.facialHairColor.r, 15), GetValidByteColor(characterCreationData.facialHairColor.g, 15), GetValidByteColor(characterCreationData.facialHairColor.b, 15), 255));
