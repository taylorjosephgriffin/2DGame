...
public class MapGenerator : MonoBehaviour
{
    ...
    void AddDirectionalPassage()
    {
        ...
        {
            int xIndex = (int)(width / 2 + .5f);
            // player.position = new Vector3Int(xIndex, 0, 0);
            Coord startTile = new Coord(xIndex, 0);
            Coord endTile = new Coord();
            Debug.Log(new Vector2(xIndex, 0));
        ...
        if (direction == EntranceDirection.EAST)
        {
            ...
            int yIndex = (int)(height / 2 + .5f);
            // player.position = new Vector3Int(width - 1, yIndex, 0);
            Coord startTile = new Coord((int)width, yIndex);
            Coord endTile = new Coord();
            for (int x = width - 1; x >= 0; x--)
            {
                if (map[x, yIndex] == 0) 
                {
                    endTile = new Coord(x, yIndex);
                    break;
                } else continue;
            }
            List<Coord> line = GetLine(startTile, endTile);
        ...
        for (int x = 0; x < width ; x++) 
        {
            ...
            for (int y = 0; y < height; y++) 
            {
                ...
                if (map[x, y] == 1) 
                {
                    ...
                    tilemap.SetTile(new Vector3Int(x, y, 0), groundTiles[pseudoRandom.Next(1, groundTiles.Length)]); 
                    if (tree != null && pseudoRandom.Next(0, 100) < treeChance && 
                        ObjectAreClearFromOtherObjects(x, y, 5) &&
                        ObjectsAreClearFromWalls(x, y)) {
                    ...
                    if (bush != null && pseudoRandom.Next(0, 100) < bushChance && 
                        ObjectAreClearFromOtherObjects(x, y, 3) &&
                        ObjectsAreClearFromWalls(x, y)) {
    ...
    bool ObjectAreClearFromOtherObjects(int gridX, int gridY, float distance)
    {
        int objectsWithinRange = 0;
        foreach (Vector2 coordinate in renderedDestructables)
        {
    ...
    bool ObjectsAreClearFromWalls(int gridX, int gridY)
    {
        ...
        for (int x = 0; x < width; x++)
        {
            for (int y = 0; y < height; y++)
            {
                if (map[x, y] == 1)
                {
                    if (Vector2.Distance(new Vector2(x, y), new Vector2(gridX, gridY)) < 3)
                    {
